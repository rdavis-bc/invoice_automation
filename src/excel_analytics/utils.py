...
class FunctionClass():
    def __init__(self, the_func: Callable[[int], None]) -> None:
        pass

def _resolver_method(self, attr):
    """
     The resolution method of attributes that will replace __
    getattr__. 
    """
    if attr.startswith("resolve_"):
        *_ignore , actual_attr = attr.partition("resolve_")
    else:
        actual_attr = attr
    try:
...
@AddGetAttrToClass()
class some_class:
    def __init__(self, tree:int) -> None:
        self.tree = 1
...
class PDFSerializer(SheetSerializer):
    ...
    def ext(self) -> str:
        return self._ext
# anti-pattern change this to using the collections.UserDict
